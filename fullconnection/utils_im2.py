...
def generate_im2_conv_data(M, N, d, wavelength, SNR, doa_min, NUM_REPEAT, grid, GRID_NUM, MC_mtx, AP_mtx, pos_para, noise_flag=True):
...
def generate_im2_conv_multi_data(M, N, d, wavelength, SNR, doa_min, NUM_REPEAT, grid, GRID_NUM, MC_mtx, AP_mtx, pos_para, noise_flag=True):
    im2_data = []
...
def generate_conv_test_data(M, N, d, wavelength, DOA, MC_mtx, AP_mtx, pos_para, coh_flag):
    K = len(DOA)
    array_signal = 0
    ...
    for ki in range(K):
        ...
        a_i = np.matmul(AP_mtx, a_i)
        a_i = np.matmul(MC_mtx, a_i)
        array_signal_i = np.matmul(a_i, signal_i) # 原本是signal_i，如果是多径就用signal_0，如果是不相干的信号源就用signal_i
...
def generate_array_cov_vector(M, N, d, wavelength, DOA, SNR, MC_mtx, AP_mtx, pos_para, co_flag):
    K = len(DOA)
    array_signal = 0
    #相干的信号源
    #signal_0 = generate_sin_signal(N)
...
def generate_im2_multi_data(M, N, d, wavelength, SNR, doa_min, NUM_REPEAT, grid, GRID_NUM, MC_mtx, AP_mtx, pos_para):
    im2_data = {}
    im2_data['nf'] = []
    im2_data['wn'] = []
    #两个函数不一样的点仅仅在于这个函数在数据集里面生成包含2-3个多径的信号，问题就在于怎么生成这个多径呢？
...
def generate_im2_data(M, N, d, wavelength, SNR, doa_min, NUM_REPEAT_SF, grid, GRID_NUM, MC_mtx, AP_mtx, pos_para):
    #这个不一定是需要同一个函数里完全出来，可以是反复利用不同的输入参数得到的，比如缺陷矩阵都是I的时候出来的就是无缺陷的情况
    im2_data = {}
    im2_data['nf'] = []
    im2_data['wn'] = []
